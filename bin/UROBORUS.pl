...
####################################################################################
	...
	{	
		...
		unlink "mapped_reads_list.txt" if -e "mapped_reads_list.txt";
		unlink "PI_list.txt" if -e "PI_list.txt";
		unlink "tmp_genome_sorted.gtf" if -e "tmp_genome_sorted.gtf";
	}
...
sub sorting_gtf
{
	...
	my $sorted_gtf;
	open(ID,$opt_gtf) || die "$!";
	open(OUT,">tmp_genome_sorted.gtf") || die "$!";
	while(<ID>){
	        if (/^#/){
	                next;
	        }
	        if (/^(\S+)\t.*\texon\t(\d+)\t(\d+)\t/){
	                ...
	                if ($chrid ne $1){
	                        my $init;
	                        my %posd;
	                        foreach my $i (sort {$post{$a} <=> $post{$b}} keys %post){
	                                my @wd = split(/\t/,$i);
	                                if (!defined($init)){
	                                        $init = $wd[3];
	                                }
	                                if ($init != $wd[3]){
	                                        foreach my $j (sort {$posd{$a} <=> $posd{$b}} keys %posd){
	                                                $chr{$chrid} .= $j;
	                                        }
	                                        $init = $wd[3];
	                                        %posd = ();
	...
	foreach my $i (sort {$a cmp $b} keys %chr){
	        print  OUT $chr{$i};
	
	}
	close(OUT);
	return("tmp_genome_sorted.gtf");
}
...
#	}
	my $chr_file;	
	if ($opt_fasta =~ /\/$/){
		$chr_file = join ("",$opt_fasta,join (".",$chrid,"fa") );
	} else {	
		$chr_file = join (".",$opt_fasta,join (".",$chrid,"fa") );#define the directiory of the chromosome.
	}
	my $total_line_num = `cat $chr_file | wc -l | awk '{print \$1;}'`;	

	my $len = `head -2 $chr_file | tail -1 | wc -m`;	
...
#	my $line;
	...
	while ( $line = <CHR_U> )
	{
		...
		if ( $count > $line_num -3 )
		{
			...
			$Uper_exon = reverse ( substr(reverse($exon_tmp), $len-$pos, 123) );
